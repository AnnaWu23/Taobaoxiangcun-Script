...
class APP(Tk):
    ...
    def showInfo(self):
        ...
        for i in range(0, len(self.afterComp)):
            form.insert('', 0, text='',values=(self.afterComp[i]['brand'], self.afterComp[i]['name_full'], self.afterComp[i]['price_now'], self.afterComp[i]['ideal_price'], self.afterComp[i]['buy_status']))
            ...
            for data in self.cartGoodInfo:
                if good['name_short'] in str(data['name_full']):
                    goodsNeedInCart.append({
                        ...
                        'brand': data['brand'],
                        'time': good['time'],
                        'price_now': float(data['price'])/100,
                        'ideal_price': float(good['ideal_price']),
                        'price_gap': float(data['price'])/100 - float(good['ideal_price']),
    ...
    def select_item(self, buyList):
        if len(buyList) == 0:
            return
        for item in buyList:
                if not item['checked']:
                    label = '//*[@id="J_Item_' + str(item['ID']) + '"]/ul/li[1]/div/div/div/label'
                    self.browser.find_element_by_xpath(label).click()
    ...
    def secKill(self):
        ...
        print(self.afterComp)
        # 若未到达60s, 则一直等待
        while not Time().time_less_than_60s(self.afterComp[0]['time']): pass
        # 到达60s，刷新现在价格
        self.refreshPrice()
        # 生成需要抢购的物品列表为buyList
        self.getBuyList()
        # 选择要抢购的物品
        self.select_item(self.buyList)
...
class Login():
    ...
    def start(self):
        ...
        sleep(15)
        # get cookie
        NetDataBase.COOKIE = self.browser.get_cookies()
    ...
    def select_all_items(self):
        WebDriverWait(self.browser, 300000000).until_not(lambda x: x.find_element_by_xpath('//*[@id="J_SelectAll1"]/div/label').is_displayed())  # 等待直到登录成功
...
class Time():
    ...
    def time_less_than_60s(self, time):
        self.time_gap(time)
        if self.gap <= 60:
            return True
        return False
    def time_gap(self, time):
        now_month=datetime.now().strftime('%Y-')
        time = str(now_month) + '-' + time

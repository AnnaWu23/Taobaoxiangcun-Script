...
class APP(Tk):
    ...
    def secKill(self):
        ...
        # 若未到达60s, 则一直等待
        while not Time().time_less_than_120s(time): pass
        # 到达60s，刷新现在价格
        self.refreshPrice()
        # 生成需要抢购的物品列表为buyList
        self.getBuyList()
    ...
    def click_buy(self, startBrowser, time):
        ...
        while True:
            try:
                ...
                pass
        if startBrowser.find_element_by_link_text('提交订单'):
...
class Proxy():
    def __init__(self):
        ...
        # p = Process(target=HttpProxy.start())
        # p.start()
        pass
    def start(self):
...
class Excel():
    def __init__(self):
        excel_path = os.path.abspath(os.getcwd()) + os.sep + '抢购信息.xlsx'
        data = xlrd.open_workbook(excel_path)
        self.table = data.sheet_by_name('1')
